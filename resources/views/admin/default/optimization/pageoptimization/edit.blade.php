    ...
    <div class="row">
        <div class="col-lg-8 mx-auto">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="add_form" class="form-horizontal" action="{{ route('page-optimization.update', $page_optimization->id) }}"
                        method="POST">
                        @csrf
                        @method('PATCH')
                        ...
                        <select class="form-control " name="s_page_id"
                                    ...
                                    @foreach ($sitepages as $sitepage)
                                        <option value="{{ $sitepage['id'] }}" {{ $sitepage['id'] == $page_optimization->s_page_id ? 'selected' : '' }}>
                                            {{ $sitepage['title'] }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-9">
                            ...
                            <textarea name="description" rows="5" class="form-control">{{ $page_optimization->description }}</textarea>
                        </div>
                        </div>
