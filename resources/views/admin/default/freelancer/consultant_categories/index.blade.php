                ...
                <div class="card-body">
                    <table class="table aiz-table mb-0">
                        ...
                        <tbody>
                            ...
                            @foreach ($consultant_categories as $key => $consultant_category)
                                <tr>
                                    <td>{{ $key + 1 + ($consultant_categories->currentPage() - 1) * $consultant_categories->perPage() }}
                                    </td>
                                    <td>{{ $consultant_category->name }}</td>
                                    @php
                                        $parent = \App\Models\ProjectCategory::where('id', $consultant_category->parent_id)->first();
                                    @endphp
                                    <td>
                                        @if ($parent != null)
                                            {{ $parent->name }}
                                    ...
                                    <td>
                                        {{ $consultant_category->description }}
                                    </td>
                                    <td>
                                        <div class="p-2 bg-secondary" style="width: fit-content;">
                                            <span class="avatar avatar-square avatar-xs">
                                                <img src="{{ custom_asset($consultant_category->photo) }}">
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-right">
                                        @can('project cat edit')
                                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                                href="{{ route('consultant-categories.edit', encrypt($consultant_category->id)) }}"
