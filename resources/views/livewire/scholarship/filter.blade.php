<div class="col-md-4 col-sm-4">
    <div class="left-sidebar">
      ...
      <div class="filter">
        <h5>{{ __('Filter') }}</h5>
        <a href="javascript:void(0)" wire:click="resetFilter" id="reset-filter">{{ __('Clear filters') }}</a>
      </div>
      ...
      <div class="study-level">
        ...
        <div class="custom-select" id="filterUniversityWrap" wire:ignore>
          <select name="university">
            @foreach ($universities as $university)
                <option value="{{ $university->id }}" {{ request()->get('country') == $country->id ? 'selected' : '' }}>
...
@push('bottom_scripts')
  <script>
    $(document).ready(function() {
      ...
      $(document).on('click', '.rounded-circle', function(e) {
        var vals = $("#selectLocationFilter").val();
