                        ...
                        <div class="form-radio-option ">
                            <div class="form-radio-title">
                                ...
                                @if(!$payment_method->is_default)
                                    <button class="make_default btn edux-btn-set-as-primary" wire:click="setPrimary({{ $payment_method->id }})">Set as primary</button>
                                @endif
                                @if(count($payment_methods) > 1)
                                    @if(!$payment_method->is_default)
                                        <button class="delete_card btn edux-btn-set-as-remove" wire:click="removeCard({{ $payment_method->id }})">Remove</button>
                                    @endif
                                @endif
                            </div>
                            @if($payment_method->is_default)
